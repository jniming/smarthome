...
public class DeviceSDK {
	 ...
	 * @param object
	 */
	public static void setSearchCallback(Object paramObject) {
		NativeCaller.SetSearchCallBack(paramObject);
	}
	 ...
	 * @return userid
	 */
	public static long createDevice(String strUsername, String strPwd,
			String strHost, int nPort, String StrDid, int NetType) {
		return NativeCaller.CreateInstance(strUsername, strPwd, strHost, nPort,
				StrDid, NetType);
	}
	 ...
	 * @return
	 */
	public static int YUV420ToRGB565(byte[] yuv, byte[] rgb, int width,
			int height) {
		return NativeCaller.YUV420ToRGB565(yuv, rgb, width, height);
	}

	public static int startRecordVideo(String filename, String forcc,
			int height, int width, int framerate) {
		return object.p2pipcam.nativecaller.NativeCaller.OpenAvi(filename,
				forcc, height, width, framerate);
	}
	...
	public static int writeVideoData(byte[] data, int len, int keyframe) {
		return object.p2pipcam.nativecaller.NativeCaller.WriteData(data, len,
				keyframe);
	}
	 ...
	 * @return
	 */
	public static int getRecordVideo(long userid, int bYear, int bMon,
			int bDay, int eYear, int eMon, int eDay) {
		return NativeCaller.SearchRecordFile(userid, bYear, bMon, bDay, 0, 0,
				0, eYear, eMon, eDay, 23, 59, 59);
	}
	 ...
	 * @return
	 */
	public static int GetCapturePicture(long UserID, String filename) {
		return NativeCaller.CapturePicture(UserID, filename);
	}
	 ...
	 * @return
	 */
	public static int GetPicture(long UserID, int ntype, String filename) {
		return NativeCaller.GetParamEx(UserID, ntype, filename);
	}
	 ...
	 * @return
	 */
	public static int StartRecord(long userid, String filename, int width,
			int height, int framerate) {
		return NativeCaller.StartRecord(userid, filename, width, height,
				framerate);
	}
	...
	public static int stopRecord(long userid) {
		return NativeCaller.StopRecord(userid);
	}
